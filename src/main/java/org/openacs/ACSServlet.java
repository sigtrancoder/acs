            ...
            try {
                ...
                try {
                    ...
                    if (reqname.equals("Inform")) {
                        ...
                        */
                        OuiMapLocal om = daoService.getOuiMapRepository().findByOui(oui);
                        if(om != null){
                            if (om.getMappedoui() == null) {
                                log(lastInform, Level.WARNING, "Oui "+oui+" is mapped to null");
                            } else {
                        ...
                        */
                        ...
                        hw = daoService.getHWModelRepository().findByOuiAndHclassAndVersion(oui, lastInform.ProductClass, lastInform.getHardwareVersion());
                        if(null == hw) {
                        	hw = new HardwareModelLocal();
                        	hw.setOui(oui);
                        	hw.setManufacturer(lastInform.Manufacturer);
                        	hw.setVersion(lastInform.getHardwareVersion());
                        	hw.setHclass(lastInform.ProductClass);
                        	hw.setDisplayName(lastInform.ProductClass.equals("") ? lastInform.Manufacturer + "." + lastInform.getHardwareVersion() : lastInform.ProductClass);
                        	daoService.getHWModelRepository().save(hw);
                        }
                        HardwareModelLocal hwold = null;
                        hwold = daoService.getHWModelRepository().findByOuiAndHclass(oui, lastInform.ProductClass);
                        /********************************************************/                       
                        ...
                        */
                        ...
                        HostsLocal host = null;
                        if (hwold != null) {
                            host = hsthome.findByHwidAndSerialno(hwold.getId(), sn);
                            if(null != host) {
                            	host.setHwid(hw.getId());
                            }
                        }
                        /****************************************************/
/* MING TODO
...
*/
                        ...
                        */
                        ...
                        host = hsthome.findByHwidAndSerialno(hw.getId(), sn);
                        if(null == host) {
                        	if (autoCreateCPE) {
                        		host = new HostsLocal();
                        		host.setHwid(hw.getId());
...
//                        countEnvelopes = OnTransferComplete((TransferComplete) msg, request, lastInform, countEnvelopes, out);
        ...
        if (cpeSentEmptyReq) {
        	System.out.println("---EmptyReq --- ");
            if (session.getAttribute(ATTR_CONFIGURATOR) == null) {
 // MING TODO               session.setAttribute(ATTR_CONFIGURATOR, RunConfigurator(request, lasthost, lastInform, transfersComplete));
        ...
        // <editor-fold defaultstate="collapsed" desc="Process requests from queue">
        {
            ...
            System.out.println("Envelopes: " + countEnvelopes + " of " + maxEnvelopes);
...
//      String fwpath = Util.getFirmwarePath(this);
      ...
      String urlServer = request.getScheme() + "://" + localAddr + ":" + request.getLocalPort() + request.getContextPath();
      Configurator cfgr = new Configurator(lastInform, host.getId(), transferComplete, fwpath, urlServer, request.getSession().getId(), daoService);
      cfgr.SetCallType(queueingType);
      cfgr.start();
      return cfgr;
   }
...
//    private int queueingType = QUEUEING_TYPE_JMS;
    ...
    private void objSendResponse(HttpSession session, Message msg) {
    	System.out.println ("in objSendResponse");
    ...
    private Message objReceiveRequest(HttpSession session, Object hwid, String sn, long w) throws JMSException {
    	System.out.println ("in objReceiveRequest");
    	Configurator c = (Configurator)session.getAttribute(ATTR_CONFIGURATOR);
        if (c == null) {
